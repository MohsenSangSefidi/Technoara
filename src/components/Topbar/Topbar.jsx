...
export default function Topbar() {
  ...
  return (
    <header className="px-4 py-4 w-full text-center shadow-lg">
      {/* Topbar */}
        ...
        <nav className="lg:w-[54rem] xl:w-[57rem]">
          <ul className="*:font-YekanBakh *:text-secondary *:font-semibold *:p-2 flex items-center justify-between">
            <li className="group border-l-2 flex pl-5 cursor-pointer relative items-center justify-between w-[22%]">
              ...
              <ul className="absolute transition-all opacity-0 w-48 top-[4.5rem] right-0 invisible hover:opacity-100 hover:visible group-hover:visible group-hover:opacity-100 *:w-full *:p-3 text-right bg-[#eef0f4]">
      ...
      <div className="flex flex-col lg:hidden">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="font-Dorna text-[1.5rem]">
            <span>تکنو</span>
            <span className="text-primary">آرا</span>
          </div>
        </div>
        ...
        {/* Mobile Menu */}
        <div className={`fixed lg:hidden transition-all duration-300 p-5 ${openMobileMenu ? "right-0" : "right-[-65rem]"} top-0 bottom-0 w-full bg-[rgba(255,255,255,0.95)]`}>
          <div onClick={() => setOpenMobileMenu(false)} className="w-full flex items-center justify-end">
